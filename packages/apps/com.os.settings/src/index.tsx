...
import './styles.css';
import type { LocaleConfig } from '@webos/kernel/types';
import type { WallpaperType } from '@webos/ui';
import type { TaskbarDisplayMode } from '@webos/ui/components/Taskbar';
...
export const Settings: React.FC<SettingsProps> = () => {
  ...
  useEffect(() => {
    ...
    if (resManager) {
      ...
      const mode = resManager.getCurrentMode();
      setCurrentResolution(mode.resolution);
      setCurrentScaling(mode.scaling);
      setAutoAdjustState(resManager.isAutoAdjust());
      setResolutionPresets(resManager.getAvailablePresets());
      setScalingPresets(resManager.getAvailableScaling());

      // 监听分辨率变化
      const unsubscribe = resManager.subscribe((newMode) => {
        setCurrentResolution(newMode.resolution);
        setCurrentScaling(newMode.scaling);
      });
  ...
  const renderDisplaySection = () => {
    const handleAutoAdjustChange = (enabled: boolean) => {
      setAutoAdjustState(enabled);
      resManagerRef.current?.setAutoAdjust(enabled);
    };

    const handleResolutionChange = (preset: { resolution: { width: number; height: number } }) => {
      resManagerRef.current?.setPresetResolution(preset);
    };

    const handleScalingChange = (scaling: number) => {
      resManagerRef.current?.setScaling(scaling);
    };
