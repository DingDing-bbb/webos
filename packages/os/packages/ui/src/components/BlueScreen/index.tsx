...
- Contact system administrator for assistance
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ marginBottom: '8px', opacity: 0.7 }}>Error Code:</div>
        <div style={{ fontSize: '24px', fontWeight: 600, letterSpacing: '2px' }}>
          {latestError?.code || 'ERR_UNKNOWN'}
        </div>
        <div style={{ marginTop: '12px', opacity: 0.8 }}>{latestError?.message}</div>
      </div>
      ...
      >
        ...
        <div style={{ fontSize: '11px', opacity: 0.7 }}>Scan for help</div>
      </div>
    </div>
  );
};
...
export const BlueScreenContainer: React.FC = () => {
  ...
  const handleRecover = () => {
    if (window.webos) {
      (
        window.webos as { errorHandler?: { recoverFromBlueScreen: () => boolean } }
      ).errorHandler?.recoverFromBlueScreen();
    }
