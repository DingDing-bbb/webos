...
### 工作原理
1. **首次访问**：Service Worker 安装并缓存所有静态资源（JS、CSS、字体、图片、语言文件）
2. **再次访问**：即使离线也能正常使用，资源全部从本地缓存加载
3. **自动更新**：检测到新版本时自动更新缓存

### 缓存策略
- **网络优先**：HTML、JS 文件（确保获取最新版本）
- **缓存优先**：静态资源（CSS、字体、图片）

### 相关文件
```
public/sw.js                     # Service Worker 实现
packages/kernel/src/core/managers/updateManager.ts  # 更新管理器
packages/ui/src/components/UpdateNotification/      # 更新通知
...
### 判断逻辑
- **主要依据**：构建时间 (`__BUILD_TIME__`)
- 版本号很少改变，所以用构建时间判断是否有更新
- 构建时间不同 = 有新版本
...
### 清除内容（彻底清除所有本地数据）
1. **localStorage** - 所有 WebOS 相关数据（用户、配置、设置等）
2. **sessionStorage** - 会话数据
3. **Cache API** - 所有 Service Worker 缓存
4. **IndexedDB** - 数据库（如有使用）
5. **Service Worker** - 注销所有注册
6. **Cookies** - 同源 cookies

### 触发方式
- 恢复模式 → Reset System (Clear All Data)
- 清除后自动重新加载，进入 OOBE

### 相关文件
```
packages/bootloader/src/index.ts  # resetSystem() 方法
packages/recovery/src/index.tsx   # 恢复模式 UI
```
